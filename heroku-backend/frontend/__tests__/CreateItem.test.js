...
describe('<CreateItem />', () => {
  ...
  it('uploads a file when changed', async () => {
    ...
    input.simulate('change', { target: { files: ['fakeloldog.jpg'] } });
    await wait();
  ...
  it('handles state updating', async () => {
    ...
    wrapper
      .find('#title')
      .simulate('change', { target: { value: 'Testing Title', name: 'title' } });
    wrapper
      .find('#price')
      .simulate('change', { target: { value: 500000, name: 'price', type: 'number' } });
    wrapper
      .find('#description')
      .simulate('change', { target: { value: 'This item is great!', name: 'description' } });

    expect(wrapper.find('CreateItem').instance().state).toMatchObject({
      title: 'Testing Title',
      price: 500000,
      description: 'This item is great!',
    });
  });
